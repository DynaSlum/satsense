...
def write_target(array, name, fname, w):
    ...
    variable = dataset.createVariable('window', 'i4', dimensions=('window_len'))
    variable[:] = [w[0].start, w[0].stop, w[0].step, w[1].start, w[1].stop, w[1].step]
...
def hog_target():
    with rasterio.open('../baseimage/section_2_sentinel_grayscale.tif') as gray:
...
def lacunarity_target():
    with rasterio.open('../baseimage/section_2_sentinel_canny_edge.tif') as canny:
