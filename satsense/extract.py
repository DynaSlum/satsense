...
def extract_feature(feature, generator):
    ...
    size = np.prod(generator.shape) * generator.windows
    i = 0
...
def extract_feature_nongridded(feature, generator):
    """Compute a single feature vector.
    ...
    classids = np.ma.zeros(np.prod(shape[:-1]))
    classids.mask = np.zeros_like(classids, dtype=bool)

    size = np.prod(generator.shape) * len(generator.windows)
    i = 0
    for window, class_id in generator:
        if window.shape[:2] not in feature.windows:
            continue
        if i % (size // 10 or 1) == 0:
            logger.info("Calculating window %i out of %i", i, size)
